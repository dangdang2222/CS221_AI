...
def logic02_03():
    ...
    ################# Write Your Code Here #########################
   #Equiv(Daughrer('$x','$y'),And(Female('$y'),Child('$x','$y')))
                    #모든 y에 대해 여자자식은 dauter이다. y가 자식 x가 부모 
...
def logic02_04():
    ...
    ################# Write Your Code Here #########################
    #모든 x에 대해서 모든 y가 할머니야
    #y가 x의 할머니다 == z가 존재하는데 x의 부모여야해(성별 필요 없음) + z의 부모는 y여야해
    #return Forall('$x',Forall('$y',Equiv(Grandmother('$x','$y'),And(Female('$y'),Parent('$x','$y')))))
    
    return Forall('$x', Forall('$y',Equiv(And(Exists('$z', And(Parent('$x', '$z'), Parent('$z', '$y'))),Female('$y'),),Grandmother('$x','$y'))))
    ################################################################
...
def number_theorem():
    ...
    #모든 x에 대해서 일단 y가 존재함 -> y가 x의 후계자라면 x랑 y는 같으면 안됌 OK -> 여기서 z가 존재함 이 상황에서 모든 z는 z가 x의 후계자라면 z는 x와 같으면 안되구 대신 y랑 같아야함 왜? 후계자는 한개니까
    #조건 : y가 x의 후계자라면 x와 y는 같지 않다
    #모든 x에 대해서 일단 y가 존재함 -> 조건(x,y) -> 여기서 z가 존재함 이 상황에서 모든 z는 조건(x,z)일때 z=y랑 같아야함 왜? 후계자는 한개니까
    def condition(x,y):
        return And(Successor(x,y),Not(Equals(x,y)))
    formulas.append(Forall('$x', Exists('$y',And(condition('$x','$y'),Forall('$z',Equiv(condition('$x','$z'),Equals('$z','$y')))))))
    #2. 각각의 숫자는 홀수 혹은 짝수이며 둘 다인 경우는 없음
    formulas.append(Forall('$x',And(Or(Even('$x'),Odd('$x')),Not(And(Even('$x'),Odd('$x'))))))
    #3. 짝수의 successor는 홀수이다. 즉 짝수+후계자 --> 홀수 
